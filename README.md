...
Name | Description
------------ | -------------
authorized_page.authorized | Created automatically when a new AuthorizedPage is "touched" (after a user signs in to the provider to allow access). Then, created automatically daily to refresh information about the AuthorizedPage (such as name, profile image, etc).
